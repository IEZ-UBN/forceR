...
## Installation
### Official release
You can install the official release of `forceR` from [CRAN](xxx):
...
#>        t       y filename                              
#>    <dbl>   <dbl> <chr>                                 
#>  1     0 0.00447 0979_cropped_ampdriftcorr_baselinecorr
#>  2     1 0.00954 0979_cropped_ampdriftcorr_baselinecorr
#>  3     2 0.0146  0979_cropped_ampdriftcorr_baselinecorr
#>  4     3 0.0146  0979_cropped_ampdriftcorr_baselinecorr
#>  5     4 0.00951 0979_cropped_ampdriftcorr_baselinecorr
#>  6     5 0.00951 0979_cropped_ampdriftcorr_baselinecorr
#>  7     6 0.00950 0979_cropped_ampdriftcorr_baselinecorr
#>  8     7 0.00949 0979_cropped_ampdriftcorr_baselinecorr
#>  9     8 0.00948 0979_cropped_ampdriftcorr_baselinecorr
#> 10     9 0.00948 0979_cropped_ampdriftcorr_baselinecorr
#> # ... with 128,991 more rows
```

```
df.1
#> [1] "0979_cropped_ampdriftcorr_baselinecorr"
```
...
#> [1] "0979_cropped_ampdriftcorr_baselinecorr"
#> [2] "0980_cropped_ampdriftcorr"             
#> [3] "0981_cropped_ampdriftcorr"             
#> [4] "0982_cropped_ampdriftcorr"             
#> [5] "1041_cropped_ampdriftcorr_baselinecorr"
#> [6] "1068_cropped_ampdriftcorr_baselinecorr"
#> [7] "1174_cropped_ampdriftcorr_baselinecorr"
#> [8] "1440_cropped_ampdriftcorr_baselinecorr"
```

The new tibble contains the measurement of all files in the selected folder.

For the sake of reproducibility to keep the package size to a minimum, we have simulated our own data using the `forceR` function `simulate_bites()`. The resulting data comes with the installation of `forceR`:
...
`df.all` has a sampling frequency of 1000 Hz. This is more than is often needed to analyse a time series. To reduce the sampling frequency of all measurements that are above a desired frequency of e.g. `200 Hz`, we can use the function `reduce.frq()`:
...
For some measurements, the automatic finding identification of peak starts and ends may fail. Changing the parameters of `find_strongest_peaks()` might help, but in some cases, the starts and ends have to be manually defined. For these cases, the function `correct_peak()` has been written. Below is an example of how to use this function with a real measurement, not the simulated data: 
...
```
```
#> [1] "Converting table to one peak per row..."
#> 100%..[1] "Rescaling finished."
```
...
```
```
#> [1] "Saving plots at ./example_data/plots/2022_04_03_rescaled_peaks_100.pdf..."
#> 
```
```{r eval=TRUE, warning=FALSE, message=FALSE, include=FALSE}
peaks.df.norm.100 <- red_peaks_100(df = peaks.df.norm, 
...
Both 4 and 8 coefficients most often were under the first 4 models that were followed by an AIC-change `<= 5%`:

![](./imgs/coeff_graphs.png){width=4in}

![](./imgs/best_coeffs.png){width=4in}

The first image shows the polynomial fits to the average force curves of species_A and species_B. 

Given the fact that we only analyzed four species (`n = 4 species * 4 coefficients = 12`) in this example, the histogram looks a bit scarce. If more species are analyzed, this may rather look like this:
...
```{r eval=TRUE, warning=FALSE, message=FALSE, include=T}
