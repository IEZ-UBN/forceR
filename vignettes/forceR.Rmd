...
Now we can convert the y-values of `df.all.200` to force data using the amplification and lever ratio info from the `classifier` with the `y_to_force()` function:
...
# add species and specimen info to df.all.200.tax
df.all.200.tax <- df.all.200.tax %>% 
  left_join(classifier %>% 
              select(species, specimen, measurement))
