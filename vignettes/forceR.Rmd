...
# we have set all plotting parameters so that they plot to the R plot device (write.PDFs = FALSE)
# and not to PDFs (write.PDFs = TRUE) so that results can be observed in real time.
# find five strongest peaks per species
peaks.df <- find_strongest_peaks(df = df.all.200.tax,
                                 no.of.peaks = 5,
                                 path.plots = path.plots,
                                 write.PDFs = FALSE) # TRUE
