...
Rühr, PT & Blanke, A (**in press**): `forceX` and `forceR`: a mobile setup and R package to measure and analyze a wide range of animal closing forces. *Methods in Ecology and Evolution* **XXX**: pp.XX-XX. doi: XXX
...
with each other. If not already the case, we strongly recommend renaming the raw data files before continuing with this `forceR` vignette.
...
For the sake of reproducibility to keep the package size to a minimum, we have simulated our own data using the `forceR` function `simulate_bites()`. The resulting data comes with the installation of `forceR`. **We will replace our previous `df.all` tibble with the internal `forceR` tibble**:
